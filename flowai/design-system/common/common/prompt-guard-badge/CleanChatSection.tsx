...
interface CleanChatSectionProps {
  enabled: boolean;
}
 ...
 * 클린 대화 섹션
 */
export function CleanChatSection({ enabled }: CleanChatSectionProps) {
      ...
      <div className='flex flex-col gap-1.5'>
        <div className='flex items-center gap-3'>
          <span className='text-foreground text-sm font-medium leading-5'>
            {t(i18n.common.prompt_guard_dialog.section_clean_chat)}
          </span>
          {enabled ? (
        ...
        <p className='text-muted-foreground text-xs leading-5'>
          {t(i18n.common.prompt_guard_dialog.section_clean_chat_description)}
        </p>
      </div>
    </div>
  );
